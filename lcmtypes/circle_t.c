/** THIS IS AN AUTOMATICALLY GENERATED FILE.  DO NOT MODIFY
 * BY HAND!!
 *
 * Generated by lcm-gen
 **/

#include <string.h>
#include "circle_t.h"

static int __circle_t_hash_computed;
static int64_t __circle_t_hash;

int64_t __circle_t_hash_recursive(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for (fp = p; fp != NULL; fp = fp->parent)
        if (fp->v == __circle_t_get_hash)
            return 0;

    const __lcm_hash_ptr cp = { p, (void*)__circle_t_get_hash };
    (void) cp;

    int64_t hash = 0xf9063917378793d7LL
         + __point_t_hash_recursive(&cp)
         + __int8_t_hash_recursive(&cp)
         + __int8_t_hash_recursive(&cp)
        ;

    return (hash<<1) + ((hash>>63)&1);
}

int64_t __circle_t_get_hash(void)
{
    if (!__circle_t_hash_computed) {
        __circle_t_hash = __circle_t_hash_recursive(NULL);
        __circle_t_hash_computed = 1;
    }

    return __circle_t_hash;
}

int __circle_t_encode_array(void *buf, int offset, int maxlen, const circle_t *p, int elements)
{
    int pos = 0, thislen, element;

    for (element = 0; element < elements; element++) {

        thislen = __point_t_encode_array(buf, offset + pos, maxlen - pos, &(p[element].center), 1);
        if (thislen < 0) return thislen; else pos += thislen;

        thislen = __int8_t_encode_array(buf, offset + pos, maxlen - pos, &(p[element].diameter), 1);
        if (thislen < 0) return thislen; else pos += thislen;

        thislen = __int8_t_encode_array(buf, offset + pos, maxlen - pos, &(p[element].confidence), 1);
        if (thislen < 0) return thislen; else pos += thislen;

    }
    return pos;
}

int circle_t_encode(void *buf, int offset, int maxlen, const circle_t *p)
{
    int pos = 0, thislen;
    int64_t hash = __circle_t_get_hash();

    thislen = __int64_t_encode_array(buf, offset + pos, maxlen - pos, &hash, 1);
    if (thislen < 0) return thislen; else pos += thislen;

    thislen = __circle_t_encode_array(buf, offset + pos, maxlen - pos, p, 1);
    if (thislen < 0) return thislen; else pos += thislen;

    return pos;
}

int __circle_t_encoded_array_size(const circle_t *p, int elements)
{
    int size = 0, element;
    for (element = 0; element < elements; element++) {

        size += __point_t_encoded_array_size(&(p[element].center), 1);

        size += __int8_t_encoded_array_size(&(p[element].diameter), 1);

        size += __int8_t_encoded_array_size(&(p[element].confidence), 1);

    }
    return size;
}

int circle_t_encoded_size(const circle_t *p)
{
    return 8 + __circle_t_encoded_array_size(p, 1);
}

int __circle_t_decode_array(const void *buf, int offset, int maxlen, circle_t *p, int elements)
{
    int pos = 0, thislen, element;

    for (element = 0; element < elements; element++) {

        thislen = __point_t_decode_array(buf, offset + pos, maxlen - pos, &(p[element].center), 1);
        if (thislen < 0) return thislen; else pos += thislen;

        thislen = __int8_t_decode_array(buf, offset + pos, maxlen - pos, &(p[element].diameter), 1);
        if (thislen < 0) return thislen; else pos += thislen;

        thislen = __int8_t_decode_array(buf, offset + pos, maxlen - pos, &(p[element].confidence), 1);
        if (thislen < 0) return thislen; else pos += thislen;

    }
    return pos;
}

int __circle_t_decode_array_cleanup(circle_t *p, int elements)
{
    int element;
    for (element = 0; element < elements; element++) {

        __point_t_decode_array_cleanup(&(p[element].center), 1);

        __int8_t_decode_array_cleanup(&(p[element].diameter), 1);

        __int8_t_decode_array_cleanup(&(p[element].confidence), 1);

    }
    return 0;
}

int circle_t_decode(const void *buf, int offset, int maxlen, circle_t *p)
{
    int pos = 0, thislen;
    int64_t hash = __circle_t_get_hash();

    int64_t this_hash;
    thislen = __int64_t_decode_array(buf, offset + pos, maxlen - pos, &this_hash, 1);
    if (thislen < 0) return thislen; else pos += thislen;
    if (this_hash != hash) return -1;

    thislen = __circle_t_decode_array(buf, offset + pos, maxlen - pos, p, 1);
    if (thislen < 0) return thislen; else pos += thislen;

    return pos;
}

int circle_t_decode_cleanup(circle_t *p)
{
    return __circle_t_decode_array_cleanup(p, 1);
}

int __circle_t_clone_array(const circle_t *p, circle_t *q, int elements)
{
    int element;
    for (element = 0; element < elements; element++) {

        __point_t_clone_array(&(p[element].center), &(q[element].center), 1);

        __int8_t_clone_array(&(p[element].diameter), &(q[element].diameter), 1);

        __int8_t_clone_array(&(p[element].confidence), &(q[element].confidence), 1);

    }
    return 0;
}

circle_t *circle_t_copy(const circle_t *p)
{
    circle_t *q = (circle_t*) malloc(sizeof(circle_t));
    __circle_t_clone_array(p, q, 1);
    return q;
}

void circle_t_destroy(circle_t *p)
{
    __circle_t_decode_array_cleanup(p, 1);
    free(p);
}

int circle_t_publish(lcm_t *lc, const char *channel, const circle_t *p)
{
      int max_data_size = circle_t_encoded_size (p);
      uint8_t *buf = (uint8_t*) malloc (max_data_size);
      if (!buf) return -1;
      int data_size = circle_t_encode (buf, 0, max_data_size, p);
      if (data_size < 0) {
          free (buf);
          return data_size;
      }
      int status = lcm_publish (lc, channel, buf, data_size);
      free (buf);
      return status;
}

struct _circle_t_subscription_t {
    circle_t_handler_t user_handler;
    void *userdata;
    lcm_subscription_t *lc_h;
};
static
void circle_t_handler_stub (const lcm_recv_buf_t *rbuf,
                            const char *channel, void *userdata)
{
    int status;
    circle_t p;
    memset(&p, 0, sizeof(circle_t));
    status = circle_t_decode (rbuf->data, 0, rbuf->data_size, &p);
    if (status < 0) {
        fprintf (stderr, "error %d decoding circle_t!!!\n", status);
        return;
    }

    circle_t_subscription_t *h = (circle_t_subscription_t*) userdata;
    h->user_handler (rbuf, channel, &p, h->userdata);

    circle_t_decode_cleanup (&p);
}

circle_t_subscription_t* circle_t_subscribe (lcm_t *lcm,
                    const char *channel,
                    circle_t_handler_t f, void *userdata)
{
    circle_t_subscription_t *n = (circle_t_subscription_t*)
                       malloc(sizeof(circle_t_subscription_t));
    n->user_handler = f;
    n->userdata = userdata;
    n->lc_h = lcm_subscribe (lcm, channel,
                                 circle_t_handler_stub, n);
    if (n->lc_h == NULL) {
        fprintf (stderr,"couldn't reg circle_t LCM handler!\n");
        free (n);
        return NULL;
    }
    return n;
}

int circle_t_subscription_set_queue_capacity (circle_t_subscription_t* subs,
                              int num_messages)
{
    return lcm_subscription_set_queue_capacity (subs->lc_h, num_messages);
}

int circle_t_unsubscribe(lcm_t *lcm, circle_t_subscription_t* hid)
{
    int status = lcm_unsubscribe (lcm, hid->lc_h);
    if (0 != status) {
        fprintf(stderr,
           "couldn't unsubscribe circle_t_handler %p!\n", hid);
        return -1;
    }
    free (hid);
    return 0;
}

